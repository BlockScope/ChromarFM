{-# LANGUAGE  QuasiQuotes #-}
{-# LANGUAGE  TemplateHaskell #-}

module Main where

import Graphics.Rendering.Chart.Easy hiding (at)
import Graphics.Rendering.Chart.Backend.Diagrams(toFile)
import GHC.Exts
import Chromar
import Env
import Params
import Photo

isLeaf :: Agent -> Bool
isLeaf (Leaf{}) = True
isLeaf _ = False

pCron :: Fluent Double
pCron = when (thr <>*> (constant 465)) adPCron `orElse` juvPCron
  where
    juvPCron = constant 30.3
    adPCron = constant 11.9

lastTa :: Multiset Agent -> Double
lastTa mix = head (sortWith Down [ta | (Leaf{i=i,m=_, a=_, ta=ta}, _) <- mix])

lastThr = Observable { name = "lastAppear",
                       gen = lastTa }

rateApp lta pc thrt =
    if thrt - lta > pc
        then 1.0 -- / 24.0
        else 0.0

nLeaves :: Multiset Agent -> Int
nLeaves mix =
    sum
        [ 1
        | (Leaf {}, _) <- mix ]

nL = Observable { name ="nLeaves",
                  gen = fromIntegral . nLeaves }

maxLeaf :: Multiset Agent -> Int
maxLeaf mix = max ((i . head) $ sortLeaves) 2
  where
    leaves = map fst $ select isLeaf mix
    sortLeaves = sortWith (Down . a) leaves

getAngle :: Int -> Int -> Int -> Double
getAngle i iMax nl
    | i <= iMax = minAngle
    | otherwise =
        minAngle +
        dAngle * (fromIntegral $ i - iMax) / (fromIntegral $ nl - iMax)

rosArea :: Multiset Agent -> Double
rosArea mix
  | nl <= 15 = sum lAreas
  | otherwise = sum $ take 13 (sortWith Down lAreas )
  where
    lAreas =
        [ a * ang
        | (Leaf {i = i
                ,m = m
                ,a = a}, _) <- mix
        , let ang = cos $ toRad (getAngle i iMax nl) ]
    nl = nLeaves mix
    iMax = maxLeaf mix
    toRad d = d / 180 * pi

rArea = Observable { name = "rosArea",
                     gen = rosArea }

raArea = Observable { name = "trueArea",
                      gen = sumM a . select isLeaf }

dassim phR ra = phR * ra * gc
  where
    gc = 86400 * 10**(-6)*12 * (1/24.0)

sla = (*) <$> constant slaCot <*> (exp <$> ((*) <$> constant slaExp <*> (thr <-*> constant 100)))
  where
    slaCot = 0.144
    slaExp = -0.002

sla' thr = slaCot * exp (slaExp*thr)
  where
    slaCot = 0.144
    slaExp = -0.002

ldem i thra thr
  | thr - thra > texp i = 0.0
  | otherwise = dem / maxDem where
    a = 3.07
    b = 5.59
    dem = ((thr - thra + 0.5) / texp i)**(a-1) *
          (1 - ((thr - thra) / texp i))**(b-1)
    maxDem = 0.0161

texp :: Int -> Double
texp i
  | i < 3 = 300
  | otherwise = 400

m2c m = m * 0.3398

c2m c = c / 0.3398

-- growth in grams of mass for a leaf with:
-- m   : mass
-- i   : rank
-- ta  : thermal time at time of birth
-- thr : current thermal time
g m i ta thr = gmax * (1/24.0)
  where
    lc = m2c m
    gmax = 0.408 * lc

mkSt :: Multiset Agent
mkSt =
    ms
        (leaves ++[ Cell{ c = initC * ra, s=initS * initC * ra}])
  where
    cotMass = cotArea / slaCot
    leaves =
        [ Leaf{ i = 1, ta = 0.0, m = cotArea/slaCot, a = cotArea}
        , Leaf{ i = 2, ta = 0.0, m = cotArea/slaCot, a = cotArea}
        ]
    ra = rosArea (ms leaves)

leafMass = Observable { name = "mass",
                        gen = sumM m . select isLeaf }

data Agent
    = Leaf { i :: Int
           , ta :: Double
           , m :: Double
           , a :: Double }
    | Cell { c :: Double, s :: Double }
    deriving (Eq, Ord, Show)

isCell (Cell{c=c}) = True
isCell _ = False

s2c :: Double -> Double
s2c s = (kStarch * s) / (24 - d)

$(return [])


----- rules -------

growth = [rule| Leaf{i=i, m=m, a=a, ta=ta}, Cell{c=c, s=s'} -->
                Leaf{m=m+(c2m $ g leafMass i ta thr), a=max a ((sla' thr) * (m+g leafMass i ta thr))},
                Cell{c=c-(g leafMass i ta thr), s=s'} @ldem i ta thr [c-(g leafMass i ta thr) > 0.0] |]

assim = [rule| Cell{c=c, s=s'} -->
               Cell{c=c + 0.875*dassim (phRate temp) rArea,
                    s=s'+ 0.125*dassim (phRate temp) rArea}
               @1.0 [day] |]

starchConv = [rule| Cell{c=c, s=s'} -->
                    Cell{c=c+(s2c s'), s=s'-(s2c s')} @1.0 [not day] |]

leafCr =
    [rule| Cell{s=s'} --> Cell{s=s'},Leaf{i=(floor nL+1), ta=thr, m=cotArea/slaCot, a=cotArea}
                                          @(rateApp lastThr pCron thr) |]
        
md = Model { rules    = [growth, assim, leafCr, starchConv],
            initState = mkSt }

carbon = Observable { name = "carbon",
                      gen  = sumM c . select isCell }

starch = Observable { name = "starch",
                      gen = sumM s . select isCell }

m1 = Observable { name = "mass1",
                  gen = sumM m . selectAttr i 1 . select isLeaf }

m2 = Observable { name = "mass2",
                  gen = sumM m . selectAttr i 2 . select isLeaf }
     
m3 = Observable { name = "mass3",
                  gen = sumM m . selectAttr i 3 . select isLeaf }


         
main = runTW md 800.0 "out/out.txt" [leafMass, rArea, carbon, nL, m1, m2]
